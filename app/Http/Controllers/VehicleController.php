...
class VehicleController extends Controller
{
    ...
    // Tambahan: high vehicles (engine_cc > 2000)
    public function highVehicles(Request $request)
    {
        try {
            $vehicles = $request->user()->vehicles()
            ...
            return response()->json([
                'status' => 'success',
                'message' => 'Kendaraan dengan engine di atas 2000cc berhasil ditemukan.',
                'data' => $vehicles,
            ]);
        } catch (\Exception $e) {
            Log::error('Error retrieving high vehicles: ' . $e->getMessage());

            return response()->json([
                'status' => 'failed',
                'message' => 'Gagal mengambil data kendaraan dengan engine besar.',
                'data' => [],
            ], 500);
        }
    }
